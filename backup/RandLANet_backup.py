...
class Network:
    def __init__(self, dataset, config):
        ...
        with tf.variable_scope('inputs'):
            ...
            K_points_numpy = np.concatenate((np.array((0, 0, 0))[None, :], K_points_numpy), axis=0)
    ...
    def inference(self, inputs, is_training):
        ...
        for i in range(self.config.num_layers):
            f_encoder_i = self.dilated_res_block(feature, inputs['xyz'][i], inputs['K_points'], 
                                                 inputs['neigh_idx'][i], d_out[i],
    ...
    def dilated_res_block(self, feature, xyz, K_points, neigh_idx, d_out, name, is_training):
        f_pc = helper_tf_util.conv2d(feature, d_out // 2, [1, 1], name + 'mlp1', [1, 1], 'VALID', True, is_training)
        f_pc = self.building_block(xyz, f_pc, K_points, neigh_idx, d_out // 2, name + 'LFA', is_training)
    ...
    def building_block(self, xyz, feature, K_points, neigh_idx, d_out, name, is_training):
        ...
        f_pc_agg = helper_tf_util.conv1d(f_xyz, d_out, 1, name + 'mlp2', 1, 'VALID', True, is_training)
